...
import { getBankrollStats } from "@/lib/finance-service";
...
export default function BankrollPage() {
  ...
  useEffect(() => {
    if (user) {
      getBankrollStats(user.id).then(data => {
        setStats(data);
        setLoading(false);
      });
    }
  ...
  const chartData = stats.history.map((h: any, idx: number) => {
    let balance = stats.history[0].bankroll_snapshot;
    for (let i = 0; i <= idx; i++) {
      balance += Number(stats.history[i].lucro_prejuizo);
    }
    return {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card className="border-none shadow-lg rounded-[2rem] bg-white p-6 flex flex-col justify-between h-40">
            ...
            <div>
              ...
              <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Pior Queda Registrada</p>
            </div>
          </Card>
        </div>
