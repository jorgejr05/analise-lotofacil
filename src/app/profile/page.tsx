        ...
        <header className="flex items-end justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button 
              ...
              variant="ghost" 
              size="icon"
              className="text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-xl h-10 w-10 transition-colors"
            >
            ...
            <Card className="border-none shadow-xl rounded-[2.5rem] bg-white dark:bg-slate-900 overflow-hidden">
              <CardContent className="p-8 flex flex-col items-center text-center space-y-4">
                <div className="relative group">
                  <Avatar className="h-32 w-32 border-4 border-slate-50 dark:border-slate-800 shadow-2xl overflow-hidden">
                ...
                <div>
                  <h2 className="text-xl font-black text-slate-900 dark:text-slate-100 italic uppercase tracking-tighter">{formData.first_name} {formData.last_name}</h2>
                </div>
              </CardContent>
            </Card>
          </div>
                ...
                <form onSubmit={handleUpdateProfile} className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 ml-1">Nome</Label>
                      <Input value={formData.first_name} onChange={(e) => setFormData({...formData, first_name: e.target.value})} className="h-12 rounded-xl border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 font-bold" />
                    </div>
                    <div className="space-y-2">
                      <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 ml-1">Sobrenome</Label>
                      <Input value={formData.last_name} onChange={(e) => setFormData({...formData, last_name: e.target.value})} className="h-12 rounded-xl border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 font-bold" />
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 pt-4 border-t border-slate-50 dark:border-slate-800">
                    ...
                    <Input 
                      ...
                      className="h-12 rounded-xl border-slate-100 dark:border-slate-800 bg-indigo-50/30 dark:bg-indigo-900/10 font-mono text-xs" 
                    />
                    <p className="text-[8px] font-bold text-slate-400 dark:text-slate-500 uppercase leading-relaxed">
                      Por segurança, a chave atual não é exibida. Para alterar, cole a nova chave acima.
                    </p>
                  </div>

                  <Button type="submit" disabled={updating} className="w-full h-14 bg-indigo-600 hover:bg-indigo-700 rounded-xl font-black uppercase italic tracking-widest text-xs shadow-lg shadow-indigo-100 dark:shadow-indigo-900/20">
              ...
              <CardContent className="p-8">
                <form onSubmit={handleUpdateSecurity} className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="h-12 rounded-xl border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 font-bold" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 ml-1">Nova Senha</Label>
                    <Input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="••••••••" className="h-12 rounded-xl border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 font-bold" />
                  </div>
                  <Button type="submit" disabled={updating} variant="outline" className="w-full h-14 border-2 border-slate-100 dark:border-slate-800 rounded-xl font-black uppercase italic tracking-widest text-xs text-slate-600 dark:text-slate-400">
